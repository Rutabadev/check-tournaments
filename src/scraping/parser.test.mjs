...
const el = (text, spots = null, hasButton = true) => ({
  innerText: text,
  spots,
  hasButton,
});
...
describe("parseTournament", () => {
  ...
  describe("level extraction", () => {
    ...
    it("handles missing level", () => {
      const result = parseTournament(el("lun. 10 jan. 18h00 homme"), "test");
  ...
  describe("day mapping", () => {
    ...
    it("maps sam. to samedi", () => {
      const result = parseTournament(el("P100 sam. 10 jan. 18h00"), "test");
      expect(result.dayOfWeek).toBe("samedi");
    });

    it("maps dim. to dimanche", () => {
      const result = parseTournament(el("P100 dim. 10 jan. 18h00"), "test");
